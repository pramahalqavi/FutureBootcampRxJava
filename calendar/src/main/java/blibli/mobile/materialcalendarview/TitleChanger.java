...
class TitleChanger {
  ...
  public TitleChanger(TextView title) {
    ...
    translate = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
        DEFAULT_Y_TRANSLATION_DP, res.getDisplayMetrics());
  }
  ...
  private void doChange(final long now, final CalendarDay currentMonth, boolean animate) {
    ...
    if (!animate) {
      ...
      viewPropertyAnimator.alpha(0).setDuration(animDuration).setInterpolator(interpolator)
          .setListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
              ...
              viewPropertyAnimator.alpha(1).setDuration(animDuration).setInterpolator(interpolator)
                  .setListener(new AnimatorListener()).start();
            }
          }).start();
    }
