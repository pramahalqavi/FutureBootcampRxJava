...
class DayView extends AppCompatCheckedTextView {
  ...
  public void setDayFormatter(DayFormatter formatter) {
    this.contentDescriptionFormatter =
        contentDescriptionFormatter == this.formatter ? formatter : contentDescriptionFormatter;
  ...
  private void setEnabled() {
    ...
    if (!isInMonth && shouldBeVisible) {
      setTextColor(
          getTextColors().getColorForState(new int[] {-android.R.attr.state_enabled}, Color.GRAY));
    }
  ...
  void applyFacade(DayViewFacade facade) {
    ...
    if (!spans.isEmpty()) {
      ...
      for (DayViewFacade.Span span : spans) {
        formattedLabel.setSpan(span.span, span.startIndex != null ? span.startIndex : 0,
            span.endIndex != null ? span.endIndex : label.length(),
